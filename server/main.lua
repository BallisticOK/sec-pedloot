...
--PLAYER ON JOIN 
RegisterNetEvent('sec-pedloot:server:onjoin', function()
    local src = source
    if SystemIsEnable then 
        TriggerClientEvent('sec-pedloot:client:enable', src) 
    end
end)

-- ENABLE LOOT SYSTEM
RegisterServerEvent('sec-pedloot:server:enable', function(eventname)
    SystemIsEnable = true
end)

-- DISABLE LOOT SYSTEM 
RegisterServerEvent('sec-pedloot:server:disable', function(eventname)
    SystemIsEnable = false
end)

-- ENABLE EVENT MODE SYSTEM
RegisterServerEvent('sec-pedloot:server:enableEvent', function()
    SystemIsEnable = true
    isAEventEnable = true
end)

-- DISABLE EVENT MODE SYSTEM 
RegisterServerEvent('sec-pedloot:server:disableEvent', function()
...
RegisterNetEvent('sec-pedloot:server:getloot', function(entity)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    TriggerClientEvent('sec-pedloot:client:deleteped', -1, entity)
...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        if SystemIsEnable then
            TriggerClientEvent('sec-pedloot:client:enable', -1)
        else
            TriggerClientEvent('sec-pedloot:client:disable', -1)
        end
    end
end)
